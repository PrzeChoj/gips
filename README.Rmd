...
We can see the standard estimator of the covariance matrix, $\hat{\Sigma} = \sum_{i=1}^n \left( Z^{(i)}\left({Z^{(i)}}^\top\right)\right)$. It is not MLE (again, because MLE does not exists):
