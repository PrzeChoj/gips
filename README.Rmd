...
plot(gips(sigma_matrix, 2), type = "heatmap") +
...
We see that the found permutation is
`r round(compare_posteriories_of_perms(g_map, g, print_output = FALSE), 2)`
times more likely than making no additional assumption. That means the
...
(`r number_of_observations`) is bigger than or equal to
$n_0 = `r summary(g_map)$n0`$, so we can estimate the covariance matrix with
...
Note that the rank of the `S` matrix was `r qr(S)$rank`, while the rank of the
`S_projected` matrix was `r qr(S_projected)$rank` (full rank).
