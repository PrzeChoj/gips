...
get_probabilities_from_gips(g_map)
```
We see that assumption $(3,4,5)$ is the most likely with $6.2\%$ posterior probability. There is 21 possible permutations that are more likely than id.

Remember that for the `n0` could still be to big for your data. In this example the assumptions with transpositions (like $(3,5)$) would yield the `n0` $= 4$, which would be insufficient for us to estimate covariance correctly. The assumption $(3,4,5)$ will be just right:
...
3. For an in-depth analysis of the package performance, capabilities, and comparison with similar packages, see an article "Learning permutation symmetries with gips in R" by `gips`' developers Adam Chojecki, Paweł Morgen, Bartosz Kołodziejek available on [arXiv:2307.00790](https://arxiv.org/abs/2307.00790).
