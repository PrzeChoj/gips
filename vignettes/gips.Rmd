...
The result depends on two input parameters, `delta` and `D_matrix`. By default they are set to `3` and `diag(p) * d`, respectively, where `d = mean(diag(S))`. The method is not scale-invariant and therefore we recommend to run gips for different values of `D_matrix` of the form `D_matrix = d * diag(p)`, where `d` $\in \mathbb{R}^+$). TODO(odwolanie do arXiv)
