...
maximum likelihood method can be used. The constraint we consider is - none other than - invariance under permutation symmetry. 
...
```
`gips` decided the $(3,4,5)$ was the most reasonable assumption. Let's see how much better it is:
...
```
We see that assumption $(3,4,5)$ is the most likely with $19.9\%$ posterior probability. However, the assumption $(3,5)$ is also quite possible with a posterior probability of $19.6\%$. So it is up to us to decide which one to choose.

Remember that with assumption $(3,5)$, the `n0` will be 4, which would be insufficient for us to estimate covariance properly. The assumption $(3,4,5)$ will be just right:
...
```
Now, the estimated covariance matrix is full rank (5).
