...
Only after 10 iterations the found permutations is unimaginably more likely than the original, `()` permutation.
...
The `show_progress_bar = TRUE` simply means `gips` will print the "=" characters on the console, as the optimization run. Keep in mind that when one sets the `return_probabilities = TRUE`, there will be a second progress bar indicating calculating the probabilities after optimization.
