...
The space of permutations is enormous - for the permutation of size $p$, the space of all permutations is of size $p!$ ($p$ factorial). Even for $p=11$, this space is practically impossible to browse. This is why `find_MAP()` implements multiple (3) optimizers to choose from:

* `"Metropolis_Hastings"`, `"MH"`
* `"hill_climbing"`, `"HC"`
* `"brute_force"`, `"BF"`, `"full"`

In general, we recommend the Brute Force for $p\le 9$.

For bigger $p>10$, we recommend Metropolis-Hastings.
...
sigma_matrix <- (function(x) {
  t(x) %*% x
})(matrix(rnorm(perm_size * perm_size), nrow = perm_size)) # the real covariance matrix, that we want to estimate
...
This is **only recommended** for small spaces ($p \le 9$). It can also browse bigger spaces, but the required time is probably too long. We tested how much time it takes to browse with Brute Force, and we show the time in the table below:

| p=2       | p=3       | p=4      | p=5     | p=6   | p=7     | p=8    | p=9   | p=10 |
|-----------|-----------|----------|---------|-------|---------|--------|-------|------|
| 0.007 sec | 0.015 sec | 0.04 sec | 0.2 sec | 1 sec | 4.5 sec | 30 sec | 4 min | TODO |
...
[2] "Learning permutation symmetries with gips in R" by `gips`' developers Adam Chojecki, Paweł Morgen, Bartosz Kołodziejek available on [arXiv:2307.00790](https://arxiv.org/abs/2307.00790).
