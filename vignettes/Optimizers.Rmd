...
The space of permutations is enormous - for the permutation of size $p$, the space of all permutations is of size $p!$ ($p$ factorial). Even for $p=10$, this space is practically impossible to browse. This is why `find_MAP()` implements multiple (3) optimizers to choose from:

* `"Metropolis_Hastings"`, `"MH"`
* `"hill_climbing"`, `"HC"`
* `"brute_force"`, `"BF"`, `"full"`

In general, we recommend the Brute Force for $p\le 8$. For bigger $p>9$, we recommend Metropolis-Hastings.

### Note on computation time

The `max_iter` parameter functions differently in Metropolis-Hastings and hill climbing.

For Metropolis-Hastings, it computes a posteriori of `max_iter` permutations, whereas for hill climbing, it computes ${p\choose 2} \cdot$ `max_iter` of them. In the case of the Brute Force optimizer, it computes all $p!$ of $f(\sigma)$ values.
...
plot(g_map, type = "best")
g_map2 <- find_MAP(g_map, max_iter = 150, optimizer = "continue")
