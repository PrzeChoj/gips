...
compared to number of variables. That's the case for 
graphical models used in biology or medicine. In such setting, the usual way
...
swapping the first with the third row, and then swapping the first and the third column, results
...
Still, if the respective values in the (poorly) estimated covariance matrix would be close to each other, adopting
a certain assumption about invariance by permutation would be a reasonable step.

## Package `gips`

We propose to create a set of constraints on the covariance matrix such, that the 
maximum likelihood method can be used after all. The constraint we consider is - none other than - 
invariance by permutation symmetry. 

This package provides a way to find a *reasonable* permutation to be used as constraint in covariance matrix estimation. In this case *reasonable* means such, that maximizes the posterior distribution when using Wishart disribution on symmetric, positive definite matrices as a prior. The idea, exact formulas and algorithm sketch are a contribution of PAPER.
