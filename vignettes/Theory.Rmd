...
round(block_decomposition, 5)
```
Again, this result is in accordance with [1,Theorem 1]. 
...
`gips` performs Bayesian model selection. The method has two parameters of the prior distribution, delta and D_matrix. By default they are set to 3 and diag(p), respectively. However, it is worth to run the procedure for several parameters D_matrix of the form $D*diag(p)$ for positive constant $D$. Large $D$ (compared to the data) favors small groups. 
