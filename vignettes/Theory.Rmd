...
gips:::pretty_plot_matrix(block_decomposition)
```
...
gips:::pretty_plot_matrix(S_projected)
```
...
2. Calculate the A Posteriori distribution (accurate to within a constant) by formula [[1,(30)]]((https://arxiv.org/abs/2004.03503.pdf)).
...
Metropolis-Hastings algorithm suggested by the authors of the [[1]](https://arxiv.org/abs/2004.03503.pdf) is one way. It is a good one. To see the discussion on it and other options available in `gips`, see `vignette("Optimizers")` or its [pkgdown page](https://przechoj.github.io/gips/articles/Optimizers.html).
...
We see the formula [1,(30)] gave the biggest value for the permutation $(1,2,3,4,5,6)$. It was over 150 times bigger than for the trivial $\text{id} = (1)(2)...(p)$ permutation. We interpret that under the assumptions (normality, the mean 0 and the A Priori), it is over 150 times more reasonable to assume the data `Z` was drawn from model $\mathrm{N}_p(0,\text{S_projected})$ than from model $\mathrm{N}_p(0,\text{S})$.
