...
$$\log L\left(\hat{\Sigma}; Z^{(1)},\ldots,Z^{(n)}\right) = \sum_{i=1}^n \left(- \frac{p}{2}\log (2\pi) - \frac{1}{2}\log\left( \det\left( \hat{\Sigma}\right)\right) - \frac12 {Z^{(i)}}^\top \hat{\Sigma}^{-1} Z^{(i)}\right)= $$

$$- \frac{np}{2}\log (2\pi) - \frac{n}{2}\log\left( \det\left( \hat{\Sigma}\right)\right) - \frac{n}2\mathrm{tr}(\hat{\Sigma}^{-1} S),$$
where $S = \frac{1}{n} \sum_{i=1}^n Z^{(i)}\cdot {Z^{(i)}}^\top$.

In `gips` we estimate $\Sigma$ as $\hat{\Sigma} = \pi_{c}(S)$. Further, for every $c$ we have $\mathrm{tr}(\pi_{c}(S)^{-1} \cdot S) = p$, so:
