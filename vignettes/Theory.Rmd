...
`gips` performs Bayesian model selection. The method has two parameters of the a prior distribution (TODO: Name of this distribution; Diaconis–Ylvisaker?), `delta` (real number, $\delta > 2$; TODO: this $\delta > 2$ is a simplification; Maybe put a note on the bottom of the page with the correct restriction?) and `D_matrix` (matrix of the same size as `S`, TODO: Additional assumptions? Full rank? Can it be the zero matrix? Negative definite?). By default they are set to 3 and `diag(p)`, respectively. However, it is worth to run the procedure for several parameters `D_matrix` of the form $D\cdot diag(p)$ for positive constant $D$. Large $D$ (compared to the data) favors small groups.
