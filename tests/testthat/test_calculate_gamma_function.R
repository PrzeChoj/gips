...
test_that('calculate_gamma_omega returns proper value',{
  ...
  # perm = (1,2)
  structure_constants <- get_structure_constants(c, 2)
...
test_that("when L is 1, G_function returns the same value as calculate_gamma_omega", {
  ...
  expect_equal(G_function(cprim, structure_constants, delta),
...
test_that("G_function example from documentation", {
  ...
  expect_true(abs(G_function(perm, structure_constants, 3) - 16.443561) < 0.0001)
})

test_that("calculate_gamma_function has desired properties", {
  expect_gt(calculate_gamma_function(permutations::id, 2, 0.50000001),
            calculate_gamma_function(permutations::id, 2, 0.5001))
  
  expect_gt(calculate_gamma_function(permutations::id, 2, 0.500000000001),
            calculate_gamma_function(permutations::id, 2, 0.50000001))
  
  expect_warning(calculate_gamma_function(permutations::id, 2, 0.5))
})
